...
const PhaseCard = props => {
  ...
  const populateTasks = () => {
    return createTaskCards(
      phase.tasks.slice(-phase.basicTasks.length)
    );
  };
  ...
  const createTaskCards = taskArray =>{
    ...
    if (phase.activePhase && phase.tasks) {
      let activeTaskIndex = phase.tasks.findIndex(
        task => !task.isItOver
      );
    ...
    return taskArray.map((task) => {
      ...
      let hideOldTasks = ""
      if (!showResetHistory){
          if (taskArray.length-(taskArray.indexOf(task)) > phase.basicTasks.length){
            hideOldTasks = "d-none"
          }
      }
    ...
    return (
      <div className="shadow p-3 mb-3 card bg-white rounded p-4 my-2">
        <div className="row pb-2">
          ...
          <div className="col-md-3">{isItOver}
