...
class User {
  ...
  getUserData(userId) {
    return this.user.get(`/user/${userId}`).then((data) => data.data);
  }
  ...
  newProject({ name, budgetNumber, client, description, type, teamMembers }) {
    return this.user
      .post("/project/new", {
        name,
        budgetNumber,
  ...
  getProject(budgetNumber) {
    return this.user.get(`/project/${budgetNumber}`).then((data) => data.data);
  }
  ...
  updateTask({ phaseId, name, assignedUser, deadline, projectId, index }) {
    return this.user
      .put(`/project/${projectId}/${phaseId}/update/${index}`, {
        name,
        assignedUser,
        deadline,
      })
  ...
  taskIsOk({ phaseId, projectId, index, spentTime, message }) {
    return this.user
      .put(`/project/${projectId}/${phaseId}/taskIsOk/${index}`, {
        spentTime,
        message,
      })
      .then((data) => data.data);
  ...
  taskIsNotOk({ phaseId, projectId, index, spentTime, message }) {
    return this.user
      .put(`/project/${projectId}/${phaseId}/taskIsNotOk/${index}`, {
        spentTime,
        message,
      })
      .then((data) => data.data);
  ...
  resetPhase({ projectId, phaseId, spentTime, message }) {
    return this.user
      .put(`/project/${projectId}/resetphase/${phaseId}`, {
        spentTime,
        message,
      })
      .then((data) => data.data);
